...
class NashStrategy(Strategy):
    ...
    def get_capacity_penalty(self, agent_capacity, goal_capacity):
        return self.CAPACITY_PENALTY_FACTOR * abs(agent_capacity - goal_capacity) / (agent_capacity)
        ...
        for agent in agents:
            ...
            for goal in goals:
                ...
                utility = goal.get_reward(
                    capacity_utilization) - grid.get_total_moving_cost(agent, goal) - self.get_capacity_penalty(agent.capacity, goal.capacity)
